...
def upgrade() -> None:
    ...
    existing = conn.execute(
        text("SELECT id FROM sys_menu WHERE path = '/system/message' LIMIT 1")
    ).fetchone()
    if existing:
        return

    menu_id = str(uuid.uuid4())
    meta_json = '{"title": "system.message.title", "icon": "mdi:message-badge"}'

    conn.execute(
        text("""
            INSERT INTO sys_menu (id, name, path, pid, type, component, status, auth_code, meta, created_at, updated_at)
            VALUES (:id, 'SystemMessage', '/system/message', :pid, 'menu', '/views/system/message/list', 1, 'system:message',
                    :meta, NOW(), NULL)
        """),
...
def downgrade() -> None:
    conn = op.get_bind()
    conn.execute(text("DELETE FROM sys_menu WHERE path = '/system/message'"))
